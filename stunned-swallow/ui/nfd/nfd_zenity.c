...
static void AddTypeToFilterName(const char *typebuf, char *filterName,
                                size_t bufsize) {
...
static void AddFiltersToCommandArgs(char **commandArgs, int commandArgsLen,
  ...
  if (!filterList || strlen(filterList) == 0) return;
...
static nfdresult_t ZenityCommon(char **command, int commandLen,
                                const char *defaultPath, const char *filterList,
                                char **stdOut) {
  if (defaultPath != NULL) {
  ...
  if (processInvokeError == COMMAND_NOT_FOUND) {
    ...
    if (exitCode == 1) result = NFD_CANCEL;
  }
...
nfdresult_t NFD_OpenDialog(const char *filterList, const nfdchar_t *defaultPath,
                           nfdchar_t **outPath) {
  int commandLen = 100;
...
nfdresult_t NFD_OpenDialogMultiple(const nfdchar_t *filterList,
  ...
  if (stdOut != NULL) {
    ...
    if (AllocPathSet(stdOut, outPaths) == NFD_ERROR) result = NFD_ERROR;
...
nfdresult_t NFD_PickFolder(const nfdchar_t *defaultPath, nfdchar_t **outPath) {
  ...
  char *stdOut = NULL;
  nfdresult_t result =
      ZenityCommon(command, commandLen, defaultPath, "", &stdOut);
