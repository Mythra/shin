// copied from:
// https://github.com/wheybags/simple_exec/blob/5a74c507c4ce1b2bb166177ead4cca7cfa23cb35/simple_exec.h

// simple_exec.h, single header library to run external programs + retrieve
// their status code and output (unix only for now)
//
// do this:
      ...
      while (1) {
        ...
        switch (bytesRead = read(childToParent[READ_FD], buffer, bufferSize)) {
          case 0:  // End-of-File, or non-blocking read.
          {
            ...
            // free any un-needed memory with realloc + add a null terminator
            // for convenience
            dataReadFromChild =
            ...
            if (returnCode != NULL) *returnCode = WEXITSTATUS(status);
          ...
          default: {
            if (dataReadFromChildUsed + bytesRead + 1 >=
                dataReadFromChildSize) {
              dataReadFromChildSize += dataReadFromChildDefaultSize;
              dataReadFromChild =
                  (char*)realloc(dataReadFromChild, dataReadFromChildSize);
            }

            memcpy(dataReadFromChild + dataReadFromChildUsed, buffer,
                   bytesRead);
...
int runCommand(char** stdOut, int* stdOutByteCount, int* returnCode,
  ...
  int retval = runCommandArray(stdOut, stdOutByteCount, returnCode,
                               includeStdErr, allArgs);
  free(allArgs);
  return retval;
}
