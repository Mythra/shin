...
static HRESULT COMInit(void) { return ::CoInitializeEx(NULL, COM_INITFLAGS); }
...
static void CopyWCharToNFDChar(const wchar_t *inStr, nfdchar_t **outStr) {
  ...
  int bytesNeeded = WideCharToMultiByte(CP_UTF8, 0, inStr, inStrCharacterCount,
                                        NULL, 0, NULL, NULL);
  assert(bytesNeeded);
  bytesNeeded += 1;
  ...
  *outStr = (nfdchar_t *)NFDi_Malloc(bytesNeeded);
  ...
  int bytesWritten = WideCharToMultiByte(CP_UTF8, 0, inStr, -1, *outStr,
...
static int CopyWCharToExistingNFDCharBuffer(const wchar_t *inStr,
                                            nfdchar_t *outPtr) {
...
static void CopyNFDCharToWChar(const nfdchar_t *inStr, wchar_t **outStr) {
  ...
  int charsNeeded =
      MultiByteToWideChar(CP_UTF8, 0, inStr, inStrByteCount, NULL, 0);
  assert(charsNeeded);
  assert(!*outStr);
  ...
  *outStr = (wchar_t *)NFDi_Malloc(charsNeeded * sizeof(wchar_t));
  ...
  int ret = MultiByteToWideChar(CP_UTF8, 0, inStr, inStrByteCount, *outStr,
                                charsNeeded);
...
static int AppendExtensionToSpecBuf(const char *ext, char *specBuf,
                                    size_t specBufLen) {
  const char SEP[] = ";";
...
static nfdresult_t AddFiltersToDialog(::IFileDialog *fileOpenDialog,
                                      const char *filterList) {
  const wchar_t WILDCARD[] = L"*.*";

  if (!filterList || strlen(filterList) == 0) return NFD_OKAY;
...
static nfdresult_t AllocPathSet(IShellItemArray *shellItems,
  ...
  for (DWORD i = 0; i < numShellItems; ++i) {
    ...
    if (!(attribs & SFGAO_FILESYSTEM)) continue;
...
static nfdresult_t SetDefaultPath(IFileDialog *dialog,
                                  const char *defaultPath) {
  if (!defaultPath || strlen(defaultPath) == 0) return NFD_OKAY;
  ...
  // Could also call SetDefaultFolder(), but this guarantees defaultPath -- more
...
nfdresult_t NFD_OpenDialog(const nfdchar_t *filterList,
                           const nfdchar_t *defaultPath, nfdchar_t **outPath) {
  ...
  if (fileOpenDialog) fileOpenDialog->Release();
...
nfdresult_t NFD_SaveDialog(const nfdchar_t *filterList,
  ...
  if (fileSaveDialog) fileSaveDialog->Release();
...
nfdresult_t NFD_PickFolder(const nfdchar_t *defaultPath, nfdchar_t **outPath) {
  ...
  // Add in FOS_PICKFOLDERS which hides files and only allows selection of
  // folders
  ...
  if (fileDialog) fileDialog->Release();
